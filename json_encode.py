...
def encode_phoneme(phoneme: Phonet) -> str:
    if isinstance(phoneme, Consonant):
        return (
            "{'type': 'phonete', 'subtype': 'consonant',"
            + " 'vocal_folds': '"
            + show_vocal_folds(phoneme.vocal_folds)
            + "',"
            + " 'place': '"
            + show_place(phoneme.place)
            + "',"
            + " 'manner': '"
            + show_manner(phoneme.manner)
            + "',"
            + " 'airstream': '"
            + show_airstream(phoneme.airstream)
            + "',"
            + " 'secondary_articulation': '"
            + show_secondary_articulation_verbosely(phoneme.secondary_articulation)
            + "'}"
        )
    elif isinstance(phoneme, Vowel):
        return (
            "{'type': 'phonete', 'subtype': 'vowel',"
            + " 'height': '"
            + show_height(phoneme.height)
            + "',"
            + " 'backness': '"
            + show_backness(phoneme.backness)
            + "',"
            + " 'rounding': '"
            + show_rounding(phoneme.rounding)
            + "',"
            + " 'vocal_folds': '"
            + show_vocal_folds(phoneme.vocal_folds)
            + "',"
            + " 'vowel_length': '"
            + show_vowel_length_verbosely(phoneme.vowel_length)
            + "'}"
        )
...
def encode_phonet_inventory(phonet_inventory: PhonetInventory) -> str:
    return (
        "{ 'type': 'phonet_inventory', 'contents': ["
        + ", ".join(map(encode_phoneme, phonet_inventory.contents))
        + "] }"
    )
