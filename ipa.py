...
from english_us_text import (
	NO_ENGLISH_DESCRIPTION_FOUND_MESSAGE,
	SORRY_UNABLE_TO_CALCULATE,
)
from grapheme_grammar import (
	split_by_phonetes,
	consonants_pulmonic,
...
def ipa_and_phonet_format(
	transcription_and_phonet: Tuple[str, Optional[Phonet]]
) -> str:
	"""
...
def ipa_text_to_phonet_list(text: str) -> List[Tuple[str, Optional[Phonet]]]:
	"""
...
def analyze_transcription(ipa_text: str) -> Optional[Phonet]:
	...
	if ipa_text == "ɧ":
		return Consonant(
			VocalFolds.VOICELESS,
			MultiPlace([Place.POSTALVEOLAR, Place.VELAR]),
			Manner.FRICATIVE,
			Airstream.PULMONIC_EGRESSIVE,
			SecondaryArticulation.NORMAL,
		)
	...
	if ipa_text[-1] == "̆":
		...
		if full_grapheme is None:
			return None
		# Make the vowel extra-short, but ..
		return to_extra_short(full_grapheme)
...
def construct_transcription_recursively(
	recursion_limit: int, recursion_level: int, phone: Phonet
) -> Optional[str]:
	"""
	...
	if (
		...
		return "ʔ" + secondary_articulation_transcription(phone.secondary_articulation)
	...
	if (
		...
		return "ɴ" + secondary_articulation_transcription(phone.secondary_articulation)
	# Trills (next line)::
	if (
		is_consonant(phone)
		and phone.vocal_folds == VocalFolds.VOICED
	...
	if (
		...
		and phone.manner == Manner.TAP_OR_FLAP
		and phone.airstream == Airstream.PULMONIC_EGRESSIVE
		and phone.secondary_articulation == SecondaryArticulation.NORMAL
	):
	...
	if (
		...
		return "ɦ" + secondary_articulation_transcription(phone.secondary_articulation)
	...
	if (
		...
		return "ɮ" + secondary_articulation_transcription(phone.secondary_articulation)
	...
	if (
		...
		return "ɰ" + secondary_articulation_transcription(phone.secondary_articulation)
	...
	if (
		...
		return "ʟ" + secondary_articulation_transcription(phone.secondary_articulation)
	# Affricates (next line):
	if (
		is_consonant(phone)
		and phone.vocal_folds == VocalFolds.VOICELESS
		and phone.place == Place.POSTALVEOLAR
		and phone.manner == Manner.AFFRICATE
		and phone.airstream == Airstream.PULMONIC_EGRESSIVE
	):
		return "t͡ʃ" + secondary_articulation_transcription(
			phone.secondary_articulation
		)
	if (
		is_consonant(phone)
		...
		return "d͡ʒ" + secondary_articulation_transcription(
			phone.secondary_articulation
		)
	if (
		is_consonant(phone)
		...
		return "p͡ɸ" + secondary_articulation_transcription(
			phone.secondary_articulation
		)
	if (
		is_consonant(phone)
		...
		return "t͜s" + secondary_articulation_transcription(
			phone.secondary_articulation
		)
	if (
		is_consonant(phone)
		...
		return "d͡z" + secondary_articulation_transcription(
			phone.secondary_articulation
		)
	if (
		is_consonant(phone)
		...
		return "k͡x" + secondary_articulation_transcription(
			phone.secondary_articulation
		)
	if (
		is_consonant(phone)
	...
	if (
		is_consonant(phone)
		and phone.vocal_folds == VocalFolds.VOICED
		and phone.place == Place.LABIAL_VELAR
		and phone.manner == Manner.APPROXIMANT
		and phone.airstream == Airstream.PULMONIC_EGRESSIVE
	):
	...
	if (
		is_consonant(phone)
		and phone.vocal_folds == VocalFolds.VOICED
		and phone.place == Place.LABIAL_PALATAL
		and phone.manner == Manner.APPROXIMANT
		and phone.airstream == Airstream.PULMONIC_EGRESSIVE
	):
	...
	if (
		is_consonant(phone)
		and phone.vocal_folds == VocalFolds.VOICELESS
		and phone.place == Place.EPIGLOTTAL
		and phone.manner == Manner.FRICATIVE
		and phone.airstream == Airstream.PULMONIC_EGRESSIVE
	):
	...
	if (
		...
		return "ʡ" + secondary_articulation_transcription(
			phone.secondary_articulation
		)  # Is the epiglottal plosive voiceless? The IPA chart
		# does not specify.
	if (
		is_consonant(phone)
		and phone.vocal_folds == VocalFolds.VOICELESS
		and phone.place == Place.ALVEOLOPALATAL
		and phone.manner == Manner.FRICATIVE
		and phone.airstream == Airstream.PULMONIC_EGRESSIVE
	):
	...
	if (
		...
		and phone.vocal_folds == VocalFolds.VOICED
		and phone.place == Place.ALVEOLAR
		and phone.manner == Manner.LATERAL_FLAP
		and phone.airstream == Airstream.PULMONIC_EGRESSIVE
	):
	...
	if (
		...
		and phone.place == Place.BILABIAL
		and phone.manner == Manner.PLOSIVE
		and phone.airstream == Airstream.CLICK
	):
	...
	if (
		...
		return "ǃ" + secondary_articulation_transcription(
			phone.secondary_articulation
		)  # Or it could be PostAlveolar.
	if (
		is_consonant(phone)
		and phone.vocal_folds == VocalFolds.VOICELESS
		and phone.place == Place.PALATOALVEOLAR
		and phone.manner == Manner.PLOSIVE
		and phone.airstream == Airstream.CLICK
	):
	...
	if (
		...
		and phone.vocal_folds == VocalFolds.VOICELESS
		and phone.place == Place.ALVEOLAR
		and phone.manner == Manner.LATERAL
		and phone.airstream == Airstream.CLICK
	):
	...
	if (
		...
		and phone.place == Place.BILABIAL
		and phone.manner == Manner.PLOSIVE
		and phone.airstream == Airstream.IMPLOSIVE
	):
	...
	if (
		...
		return "ʛ" + secondary_articulation_transcription(phone.secondary_articulation)
	...
	if (
		...
		and phone.vocal_folds == VocalFolds.VOICED
		and phone.vowel_length == VowelLength.NORMAL
		and phone.vowel_length == VowelLength.NORMAL
	):
		return "i"
	if (
	...
	if (
		...
		result = construct_transcription_recursively(
			recursion_limit, 1 + recursion_level, to_alveolar(phone)
		)
		if result is None:
			return None
	...
	if (
		...
		result = construct_transcription_recursively(
			recursion_limit, 1 + recursion_level, to_normal_length(phone)
		)
		if result is None:
			return None
	...
	if (
		...
		result = construct_transcription_recursively(
			recursion_limit, 1 + recursion_level, to_voiced(phone)
		)
		if result is None:
			return None
	...
	if (
		...
		result = construct_transcription_recursively(
			recursion_limit, 1 + recursion_level, to_voiceless(phone)
		)
		if result is None:
			return None
	...
	if (
		is_consonant(phone)
		and phone.vocal_folds == VocalFolds.VOICELESS_ASPIRATED
		and phone.airstream == Airstream.PULMONIC_EGRESSIVE
		and recursion_level < recursion_limit
	):
		result = construct_transcription_recursively(
	...
	if (
		is_consonant(phone)
		and phone.vocal_folds == VocalFolds.CREAKY_VOICED
		and phone.airstream == Airstream.PULMONIC_EGRESSIVE
		and recursion_level < recursion_limit
	):
		result = construct_transcription_recursively(
			recursion_limit, 1 + recursion_level, decreak(phone)
		)
...
tone_and_word_accents: List[str] = [  # Level:
	"˥",
